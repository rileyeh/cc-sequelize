...
form.addEventListener('submit', (e) => {
    ...
    axios.post('http://localhost:9876/appt', {date: dateTimeObj, service: servicePicker.value})
        .then(res => {
            resetFormValues()
            let displayDate = createDisplayDate(res.data[0].date)
            const newPara = document.createElement('p')
            const closeBtn = document.createElement('button')

            newPara.textContent = `Your appointment request for ${displayDate} is pending!`
            newPara.className = 'notification'

            closeBtn.textContent = 'X'

            newPara.appendChild(closeBtn)

            closeBtn.addEventListener('click', () => {
                newPara.remove()
            })

            document.querySelector('#notify').appendChild(newPara)
